    ...
    render() {
        ...
        if (proposer){
            ...
            return <Row className="block-info">
                ...
                <Col xs={8} sm={3} md={2} lg={3} className="text-truncate" style={{ color: "#000000" }}><Link style={{ color: "#000000" }} to={"/validator/" + proposerOperatorAddress}><Avatar moniker={moniker} profileUrl={proposer.profile_url} address={proposerOperatorAddress} list={true} /> {moniker}</Link></Col>
                <Col xs={4} sm={1} md={homepage ? 1 : 2}><i className="fas fa-sync d-sm-none"></i> {numbro(this.props.block.transNum).format('0,0')}</Col>
                <Col xs={{size:4, offset:8}} sm={{size:2, offset:0}}><i className="fas fa-database d-sm-none"></i> <Link style={{ color: "#000000" }} to={"/blocks/"+this.props.block.height}>{numbro(this.props.block.height).format('0,0')}</Link></Col>
            </Row>
        }
        else{
